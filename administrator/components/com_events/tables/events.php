...
class EventsTableevents extends JTable {
    ...
    public function __construct(&$db) {
        parent::__construct('#__events_main', 'id', $db);
    }
    ...
    public function bind($array, $ignore = '') {
		...
		if(($task == 'save' || $task == 'apply') && (!JFactory::getUser()->authorise('core.edit.state','com_events.events.'.$array['id']) && $array['state'] == 1)){
			$array['state'] = 0;
		}
				...
				if(isset($_FILES['jform']['name']['image'])):
					...
					if($fileError > 0 && $fileError != 4) {
						...
						$uploadPath = JPATH_ADMINISTRATOR.DIRECTORY_SEPARATOR.'components'.DIRECTORY_SEPARATOR.'com_events'.DIRECTORY_SEPARATOR.'/img'.DIRECTORY_SEPARATOR.$filename;
        ...
        if(!JFactory::getUser()->authorise('core.admin', 'com_events.events.'.$array['id'])){
            $actions = JFactory::getACL()->getActions('com_events','events');
            $default_actions = JFactory::getACL()->getAssetRules('com_events.events.'.$array['id'])->getData();
            $array_jaccess = array();
    ...
    */
    protected function _getAssetName() {
        $k = $this->_tbl_key;
        return 'com_events.events.' . (int) $this->$k;
    }
